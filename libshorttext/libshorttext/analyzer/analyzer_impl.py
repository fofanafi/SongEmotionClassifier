...
Selectors: \n-> {selectors}'''\
        ...
        def get_confusion_table(self, pred_insts, confusion_table = None):
                if pred_insts.quantity is None:
                        self._calculate_info(pred_insts)
	...
	def gen_confusion_table(self, pred_insts, output = None):
		...
		on the screen.
			...
			Art          6      0  162
		'''
		confusion_table = self.get_confusion_table(pred_insts)
		self.draw_confusion_table(pred_insts, confusion_table, output)

	def draw_confusion_table(self, pred_insts, confusion_table, output = None):
                if isinstance(output, str):
                        output = open(output, 'w')
                labels = pred_insts.true_labels.union(pred_insts.predict_labels)
                for (idx, label) in enumerate(labels):
                  total = sum(confusion_table[idx])
                  acc = float(confusion_table[idx][idx]) / total if total > 0 else 0
                  write(label + ' ' + str(acc), output)
